 ...
 13050  0.3447 :        if key < self.v.len() {
               :            match self.v[key] {
 63675  1.6821 :              Some(ref value) => Some(value),
               :              None => None
               :            }
               :        } else {
               :            None
   ...
   107  0.0028 :        if key < self.v.len() {
               :            match *(&mut self.v[key]) {
  2020  0.0534 :              Some(ref mut value) => Some(value),
               :              None => None
               :            }
               :        } else {
               :            None
               ...
               :    /// assert_eq!(map[37], "c");
               :    /// ```
   567  0.0150 :    pub fn insert(&mut self, key: usize, value: V) -> Option<V> { /* _$LT$vec_map..VecMap$LT$V$GT$$GT$::insert::hc6fd9233689a0a67 total:   2033  0.0537 */
               :        let len = self.v.len();
     7 1.8e-04 :        if len <= key {
               :            self.v.extend((0..key - len + 1).map(|_| None));
               :        }
               :        let was = replace(&mut self.v[key], Some(value));
    85  0.0022 :        if was.is_none() {
               :            self.n += 1;
               :        }
   104  0.0027 :        was
   159  0.0042 :    }
   ...
   587  0.0155 :        if self.contains_key(key) {
               ...
               :    /// entry.
  1824  0.0482 :    pub fn or_insert_with<F: FnOnce() -> V>(self, default: F) -> &'a mut V { /* _$LT$vec_map..Entry$LT$$u27$a$C$$u20$V$GT$$GT$::or_insert_with::hf976055bf2efbefb total:   7861  0.2077 */
               :        match self {
   341  0.0090 :            Occupied(entry) => entry.into_mut(),
               :            Vacant(entry) => entry.insert(default()),
               :        }
  1110  0.0293 :    }
               :}
  ...
  1585  0.0419 :        self.map.insert(index, value);
               :        &mut self.map[index]
               :    }
               :}
               :
    ...
    26 6.9e-04 :        map.extend(iter);
     ...
     3 7.9e-05 :        self.get(i).expect("key not present")
               :    }
               :}
               :
   ...
   317  0.0084 :        self.get_mut(i).expect("key not present")
               :    }
               :}
               :
    ...
    18 4.8e-04 :                while self.front < self.back {
               :                    match self.iter.next() {
               :                        Some(elem) => {
               :                            match elem$(. $getter ())+ {
    66  0.0017 :                                Some(x) => {
               :                                    let index = self.front;
     4 1.1e-04 :                                    self.front += 1;
     2 5.3e-05 :                                    self.yielded += 1;
               :                                    return Some((index, x));
               :                                },
               :                                None => {},
               :                            }
     ...
     6 1.6e-04 :                    self.front += 1;
               ...
               :    type Item = usize;
               :
     1 2.6e-05 :    fn next(&mut self) -> Option<usize> { self.iter.next().map(|e| e.0) }
               :    fn size_hint(&self) -> (usize, Option<usize>) { self.iter.size_hint() }
               :}
               :
               ...
               :    type Item = &'a V;
               :
     1 2.6e-05 :    fn next(&mut self) -> Option<(&'a V)> { self.iter.next().map(|e| e.1) }
               :    fn size_hint(&self) -> (usize, Option<usize>) { self.iter.size_hint() }
               :}
               :
    ...
    83  0.0022 :                Some((i, Some(value))) => {
               ...
               :}
/* 
 * Total samples for file : "/home/gerwin/.cargo/registry/src/github.com-1ecc6299db9ec823/vec_map-0.8.0/src/lib.rs"
 * 
 *  85748  2.2651
 */
