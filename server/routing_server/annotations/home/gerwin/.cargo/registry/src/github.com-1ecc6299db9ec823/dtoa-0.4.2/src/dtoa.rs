     ...
     1 2.6e-05 :    let wp_w = mp - w;
     1 2.6e-05 :    let mut p1 = (mp.f >> -one.e) as u32;
     1 2.6e-05 :    let mut p2 = mp.f & (one.f - 1);
     ...
     1 2.6e-05 :    while kappa > 0 {
               ...
               :            _ => {}
               :        }
     1 2.6e-05 :        if d != 0 || len != 0 {
     2 5.3e-05 :            *buffer.offset(len) = b'0' + d as u8;
               :            len += 1;
               :        }
               :        kappa -= 1;
     2 5.3e-05 :        let tmp = (p1 as $sigty << -one.e) + p2;
     4 1.1e-04 :        if tmp <= delta {
               :            k += kappa as isize;
               ...
               :        if d != 0 || len != 0 {
     1 2.6e-05 :            *buffer.offset(len) = b'0' + d;
               :            len += 1;
               :        }
               :        p2 &= one.f - 1;
               :        kappa = kappa.wrapping_sub(1);
     1 2.6e-05 :        if p2 < delta {
               :            k += kappa as isize;
     ...
     2 5.3e-05 :    let (w_m, w_p) = v.normalized_boundaries();
               :
               :    let (c_mk, k) = get_cached_power(w_p.e);
     5 1.3e-04 :    let w = v.normalize() * c_mk;
     1 2.6e-05 :    let mut wp = w_p * c_mk;
     2 5.3e-05 :    let mut wm = w_m * c_mk;
               :    wm.f += 1;
               :    wp.f -= 1;
     3 7.9e-05 :    digit_gen(w, wp, wp.f - wm.f, buffer, k)
               :}
               :
               :/*
               ...
               :    else if 0 < kk && kk <= 21 {
               :        // 1234e-2 -> 12.34
     1 2.6e-05 :        ptr::copy(buffer.offset(kk), buffer.offset(kk + 1), (length - kk) as usize);
               :        *buffer.offset(kk) = b'.';
     ...
     2 5.3e-05 :    if value == 0.0 {
               ...
               :}}
/* 
 * Total samples for file : "/home/gerwin/.cargo/registry/src/github.com-1ecc6299db9ec823/dtoa-0.4.2/src/dtoa.rs"
 * 
 *     32 8.5e-04
 */
