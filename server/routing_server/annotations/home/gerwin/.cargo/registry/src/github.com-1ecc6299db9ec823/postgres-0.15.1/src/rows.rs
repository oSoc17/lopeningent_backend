               ...
               :    fn deref(&self) -> &T {
               :        match *self {
    14 3.7e-04 :            MaybeOwned::Borrowed(s) => s,
               ...
               :    type Item = Row<'a>;
               :
     5 1.3e-04 :    fn next(&mut self) -> Option<Row<'a>> { /* _$LT$postgres..rows..Iter$LT$$u27$a$GT$$u20$as$u20$core..iter..iterator..Iterator$GT$::next::hba7cbf894b94a61d total:     12 3.2e-04 */
     ...
     1 2.6e-05 :    }
               ...
               :    /// }
               :    /// ```
    35 9.2e-04 :    pub fn get<I, T>(&self, idx: I) -> T /* postgres::rows::Row::get::h09eebc3c03ecc731     28 7.4e-04, postgres::rows::Row::get::h2e24f2d05d2c67ba    220  0.0058, postgres::rows::Row::get::h92ce594e960ccc4d      5 1.3e-04, total:    253  0.0067 */
               :    where
               :        I: RowIndex + fmt::Debug,
               :        T: FromSql,
               :    {
    35 9.2e-04 :        match self.get_inner(&idx) {
    34 9.0e-04 :            Some(Ok(ok)) => ok,
    ...
    14 3.7e-04 :    }
               ...
               :        };
               :
    16 4.2e-04 :        let ty = self.stmt_info.columns[idx].type_();
     8 2.1e-04 :        if !<T as FromSql>::accepts(ty) {
               :            return Some(Err(error::conversion(Box::new(WrongType::new(ty.clone())))));
               :        }
    13 3.4e-04 :        let value = FromSql::from_sql_nullable(ty, self.data.get(idx));
     9 2.4e-04 :        Some(value.map_err(error::conversion))
     2 5.3e-05 :    }
               :
               ...
               :}
/* 
 * Total samples for file : "/home/gerwin/.cargo/registry/src/github.com-1ecc6299db9ec823/postgres-0.15.1/src/rows.rs"
 * 
 *    186  0.0049
 */
