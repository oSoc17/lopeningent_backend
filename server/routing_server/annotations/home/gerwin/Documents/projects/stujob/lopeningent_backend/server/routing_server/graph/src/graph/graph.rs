     ...
     3 7.9e-05 :            data.insert(id as usize, Element {v : vertex, links : BTreeMap::new()});
               :        }
    36 9.5e-04 :        for (id, edge, to) in edges {
     1 2.6e-05 :            try!(data.get_mut(id as usize).ok_or(Error::MissingID)).links.insert(to, edge);
               :        }
               :        Ok(Graph {
               :            data : data,
               :        })
   ...
   439  0.0116 :        return self.data.get(index as usize).map(|e| &e.v)
               :    }
               :
    ...
    93  0.0025 :            .map(|e| e.links.iter())
     ...
     1 2.6e-05 :        self.data.get(from as usize).and_then(|el| el.links.get(&to))
               :    }
               :
               ...
               :}
/* 
 * Total samples for file : "/home/gerwin/Documents/projects/stujob/lopeningent_backend/server/routing_server/graph/src/graph/graph.rs"
 * 
 *    573  0.0151
 */
